...
/** True when authentication is active (federated or local email/password). */
export const authConfigured =
  !authDisabled &&
  (Boolean(grokClientId && grokClientSecret) || emailAndPasswordEnabled);
...
export const SESSION_TOKEN_COOKIE = isProduction ? "__Host-grok-auth.session_token" : "grok-auth.session_token";
...
export const auth = betterAuth({
  ...
  // Transactional Email Verification via Brevo (Sendinblue)
  emailVerification: {
    sendOnSignUp: true,
    autoSignInAfterVerification: true,
    sendVerificationEmail: async ({ user, url, token }) => {
      try {
        const { sendBrevoVerificationEmail } = await import("@/lib/email/brevo.server");
        await sendBrevoVerificationEmail({
          toEmail: user.email,
          toName: user.name || "Citizen",
          verifyUrl: url,
          token,
        });
      } catch (err) {
        console.error("Failed to send Brevo verification email:", err);
      }
    },
  },
        ...
        emailAndPassword: {
          ...
          sendResetPassword: async ({ user, url, token }) => {
            try {
              ...
              console.error("Failed to send Brevo reset email:", err);
            }
          },
        },
      }
  ...
  advanced: {
    ...
    defaultCookieAttributes: { secure: isProduction, sameSite: "lax", path: "/" },
    cookies: {
      session_token: { name: SESSION_TOKEN_COOKIE },
      session_data: { name: isProduction ? "__Host-grok-auth.session_data" : "grok-auth.session_data" },
      account_data: { name: isProduction ? "__Host-grok-auth.account_data" : "grok-auth.account_data" },
      dont_remember: { name: isProduction ? "__Host-grok-auth.dont_remember" : "grok-auth.dont_remember" },
    },
  },
