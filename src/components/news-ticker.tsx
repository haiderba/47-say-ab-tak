...
import { Flame, ChevronLeft, ChevronRight, Zap, Radio } from "lucide-react";
...
export function NewsTicker() {
    ...
    getAggregatedNews({ data: "all" })
      .then((res) => {
        if (isMounted && res && res.articles.length > 0) {
          setArticles(res.articles.slice(0, 15));
        }
      })
      .catch(() => {});
    return () => {
  ...
  return (
    <div
      ...
      className="relative z-30 border-b border-primary/20 bg-[#013516] text-white shadow-sm transition-colors duration-300"
    >
